    ...
    <main style={{ minHeight: "100vh", padding: "32px 20px" }}>
      <div style={{ maxWidth: 1180, margin: "0 auto" }}>
        <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, paddingBottom: 28 }}>
          <div>
            <div style={{ fontWeight: 800, fontSize: 18 }}>AI Content Studio</div>
            <div style={{ color: "#7186a0", fontSize: 12, marginTop: 3 }}>Create. Edit. Publish.</div>
          </div>
          <button onClick={() => setShowCreator(true)} style={primaryButton}>+ New Video</button>
        </header>
        ...
        {!showCreator ? (
          <>
            <section style={{ border: "1px solid #1d3048", background: "linear-gradient(135deg, #0b1829, #0a1422)", borderRadius: 22, padding: "42px 30px", marginBottom: 24 }}>
              ...
              <h1 style={{ fontSize: "clamp(38px, 6vw, 68px)", lineHeight: 1, letterSpacing: "-0.05em", maxWidth: 760, margin: "20px 0 18px" }}>
                Turn your idea into a finished video.
              </h1>
              <p style={{ maxWidth: 680, color: "#a9b9cc", fontSize: 17, lineHeight: 1.7, margin: 0 }}>
                Start with a simple topic. AI Content Studio will guide you from script to voice, scenes, captions, and eventually MP4 export.
              </p>
              <button onClick={() => setShowCreator(true)} style={{ ...primaryButton, marginTop: 26, padding: "15px 22px" }}>Create your first video →</button>
            </section>
            ...
            <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))", gap: 14, marginBottom: 28 }}>
              {features.map((feature) => (
                <article key={feature.title} style={card}>
                  <h2 style={{ fontSize: 16, margin: "0 0 9px" }}>{feature.title}</h2>
            ...
            <section>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <h2 style={{ fontSize: 20, margin: 0 }}>Recent projects</h2>
                <span style={{ color: "#7186a0", fontSize: 13 }}>{projects.length} projects</span>
              </div>
              <div style={{ display: "grid", gap: 10 }}>
                {projects.map((project) => (
                  <div key={project.title} style={{ ...card, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 15 }}>
                    <div>
                      <div style={{ fontWeight: 650 }}>{project.title}</div>
                      {project.date && <div style={{ color: "#7186a0", fontSize: 12, marginTop: 5 }}>{project.date}</div>}
                    </div>
                    <span style={status}>{project.status}</span>
                  </div>
                ))}
              </div>
            </section>
          ...
          <section>
            ...
            <div style={{ maxWidth: 850, margin: "28px auto 60px" }}>
              ...
              <div style={{ ...card, marginTop: 24 }}>
                ...
                <button disabled={!topic.trim()} onClick={createProject} style={{ ...primaryButton, width: "100%", marginTop: 22, opacity: topic.trim() ? 1 : 0.45 }}>Create Video Project →</button>
              </div>
