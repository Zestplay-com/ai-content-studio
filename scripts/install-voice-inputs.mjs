...
const voiceStatePattern = /const \[voiceovers, setVoiceovers\] = useState<Record<number, \{ audio_url: string; voice: string; voice_style: string \}\>>\(\{\}\);/;
...
const functionBlock = `  async function audioFileToDataUrl(file: File) {
  ...
  async function importVoiceoverFile(sceneNumber: number, file: File) {
    ...
    try {
      ...
      setVoiceovers((current) => ({ ...current, [sceneNumber]: {
        audio_url: audioUrl,
    ...
    try {
      ...
      recorder.onstop = async () => {
        try {
          ...
          const audioUrl = await audioFileToDataUrl(new File([blob], "my-voice-recording", { type: blob.type }));
          setVoiceovers((current) => ({ ...current, [sceneNumber]: {
            audio_url: audioUrl,
...
const ui = `
                  <div style={{ marginBottom: 18, padding: 16, borderRadius: 14, background: "#081522", border: "1px solid #29476a" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10, marginTop: 14 }}>
                      ...
                      <div style={{ padding: 13, border: "1px solid #1d3048", borderRadius: 12, background: "#0b1b2c" }}><div style={{ fontWeight: 750 }}>🎙️ Record My Voice</div><div style={{ color: "#7186a0", fontSize: 11, marginTop: 4 }}>Record directly from your phone or computer microphone.</div></div>
                    </div>
...
const sceneControls = `
                          <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 8 }}>
                            <label style={{ ...secondaryButton, cursor: "pointer", textAlign: "center" }}>📁 Import Voice<input type="file" accept="audio/*,.mp3,.wav,.m4a,.ogg,.aac" style={{ display: "none" }} onChange={(event) => { const file = event.target.files?.[0]; if (file) void importVoiceoverFile(scene.number, file); event.currentTarget.value = ""; }} /></label>
                            <button type="button" onClick={() => recordingScene === scene.number ? stopVoiceRecording() : void recordVoiceover(scene.number)} disabled={recordingScene !== null && recordingScene !== scene.number} style={{ ...secondaryButton, opacity: recordingScene !== null && recordingScene !== scene.number ? 0.45 : 1 }}>{recordingScene === scene.number ? "⏹ Stop Recording · " + recordingSeconds + "s" : "🎙️ Record My Voice"}</button>
                          </div>
`;
text = text.replace(sceneArticleAnchor, sceneArticleAnchor + sceneControls);

const audioMetaPattern = /Voice: \{audio\.voice\} · Style: \{audio\.voice_style\}/;
if (!audioMetaPattern.test(text)) throw new Error("Voiceover audio metadata anchor was not found.");
text = text.replace(audioMetaPattern, 'Source: {audio.source === "upload" ? "Imported file" : audio.source === "recording" ? "My recording" : "AI"} · {audio.file_name || audio.voice}');
