...
if (text.includes("FOOTAGE_PICKER_V2")) {
...
const picker = `              {/* FOOTAGE_PICKER_V2 */}
                ...
                <section style={{ ...card, marginTop: 18, borderColor: "#29476a" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap", marginBottom: 8 }}>
                    <div>
                      ...
                      <p style={{ color: "#7186a0", fontSize: 13, margin: "5px 0 0", lineHeight: 1.5 }}>Review the AI recommendations and choose the exact clip that should be used for each scene.</p>
                    </div>
                              ...
                              return (
                                <div key={result.id} style={{ border: isSelected ? "2px solid #6ea8fe" : "1px solid #1d3048", borderRadius: 12, overflow: "hidden", background: "#0b1829" }}>
                                  <div style={{ position: "relative", aspectRatio: "16 / 9", background: "#050b12" }}>
                                    {result.video_url ? <video controls preload="metadata" poster={result.thumbnail} src={result.video_url} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /> : <img src={result.thumbnail} alt={\`Scene \${match.scene_number} footage option \${index + 1}\`} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />}
                                  ...
                                  <div style={{ padding: 10 }}>
                                    ...
                                    <button type="button" onClick={() => { setSelectedClips((current) => ({ ...current, [match.scene_number]: result.id })); setShowVoiceoverStage(false); }} style={{ ...(isSelected ? primaryButton : secondaryButton), width: "100%", fontSize: 12 }}>{isSelected ? "✓ Selected" : "Use This Clip"}</button>
                                  </div>
                                </div>
                              );
                            })}
                  ...
                  <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid #1d3048", display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
                    ...
                    <button type="button" onClick={() => { setShowVoiceoverStage(true); window.requestAnimationFrame(() => document.getElementById("voiceover-section")?.scrollIntoView({ behavior: "smooth", block: "start" })); }} disabled={Object.keys(selectedClips).length < stockMatches.length} style={{ ...primaryButton, opacity: Object.keys(selectedClips).length < stockMatches.length ? 0.45 : 1 }}>
...
if (!text.includes(marker)) throw new Error("Could not find the stock error insertion point in app/page.tsx");
